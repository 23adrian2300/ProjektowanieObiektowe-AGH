...
import pl.edu.agh.dronka.shop.model.provider.Create.Creator;
		...
		try {
			...
			for (String[] dataLine : data) {
				...
				boolean isSecondhand = Boolean.parseBoolean(reader.getValue(
						dataLine, "Używany"));
				ICreate ICreate = Creator.create(category);

				Item item = ICreate.createItem(name, category, price, quantity, dataLine, reader);
